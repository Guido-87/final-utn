...
app.controller('controlGrafica', function($scope,$http,$auth,$state) { 
	...
	else if($auth.isAuthenticated() && $auth.getPayload().perfil!="admin")
	{
...
app.controller('controlAlta', function($scope,$auth, $http ,$state,FileUploader) {  
	if($auth.isAuthenticated() && $auth.getPayload().perfil == "admin")
	{
...
//	cargadoDeFoto.CargarFoto($scope.persona.foto,$scope.uploader); 
	...
	$scope.Guardar=function(){
		...
		if($scope.uploader.queue[0].file.name!='porDefecto.jpg')
		{
...
//	FUNCION GUARDAR DEL MODIFICAR
	$scope.Guardar = function(){
		...
		$scope.uploader.onSuccessItem = function(fileItem, response, status, headers) {
			...
			$http.post('datos/modificar', { persona:$scope.persona})
			.then(function(respuesta) {       
				//aca se ejetuca si retorno sin errores
...
app.controller('controlGrillaPedidos', function($scope, $auth,$http,$location,$state) {
	...
	//$scope.pedido={};
	//$scope.estado2=$scope.ListadoPedidos[0].estado;
